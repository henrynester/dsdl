...
uint32_t local_SPINORCommand_encode(struct local_SPINORCommand* msg, uint8_t* buffer
    ...
    uint32_t bit_ofs = 0;
    memset(buffer, 0, LOCAL_SPINORCOMMAND_MAX_SIZE);
    _local_SPINORCommand_encode(buffer, &bit_ofs, msg, 
#if CANARD_ENABLE_TAO_OPTION
    tao
#else
    true
...
#if CANARD_ENABLE_TAO_OPTION
    if (transfer->tao && (transfer->payload_len > LOCAL_SPINORCOMMAND_MAX_SIZE)) {
        return true; /* invalid payload length */
    }
#endif
    uint32_t bit_ofs = 0;
    if (_local_SPINORCommand_decode(transfer, &bit_ofs, msg,
#if CANARD_ENABLE_TAO_OPTION
    transfer->tao
#else
    true
...
struct local_SPINORCommand sample_local_SPINORCommand_msg(void) {
    struct local_SPINORCommand msg;

    msg.command_type = (uint8_t)random_bitlen_unsigned_val(8);
    msg.command = random_float_val();
    return msg;
}
#endif
